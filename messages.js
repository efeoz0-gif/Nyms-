...
import {
  ...
  onSnapshot, serverTimestamp, query, orderBy, limit, arrayUnion, arrayRemove
...
// Bot/sistem mesajı gönderir — authorUid her zaman "bot_" ile başlar (Rules bunu ayrı tanır).
// Botu tetikleyen kullanıcının kendi client'ı bu fonksiyonu çağırır (7/24 sunucu gerektirmez).
export async function togglePin(messagesRef, messageId, pinned) {
  await updateDoc(doc(messagesRef, messageId), { pinned });
}

export async function sendBotMessage(messagesRef, bot, content, extra = {}) {
  const ref = await addDoc(messagesRef, {
    authorUid: `bot_${bot.id || "system"}`,
    authorUsername: bot.name || "Bot",
    authorTag: "BOT",
    authorPhoto: null,
    authorAvatarEmoji: bot.avatarEmoji || "🤖",
    content,
    editedAt: null,
    replyToId: null,
    replyToPreview: null,
...
// Etkileşimli bot mesajlarını (ör. Blackjack "Kart Çek"/"Dur") güncellemek için
export async function updateBotMessage(messagesRef, messageId, patch) {
  await updateDoc(doc(messagesRef, messageId), patch);
}
...
export async function sendMessage(messagesRef, content, { replyToId = null, replyToPreview = null } = {}) {
  ...
  await addDoc(messagesRef, {
    ...
    authorNameColor: profile.nameColor || null,
    authorAvatarFrame: profile.avatarFrame || null,
    content,
    editedAt: null,
    replyToId,
...
// Son N mesajı bir kereliğine çeker (canlı dinlemez) — /sil komutu için
export async function getRecentMessages(messagesRef, count) {
