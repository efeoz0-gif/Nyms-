...
export async function createOrRefreshProfile(user, { displayNameOverride } = {}) {
  ...
  if (!existing.exists()) {
    ...
    await setDoc(userRef, {
      ...
      isBanned: false,
      isMuted: false,
      cash: 2000, // yeni hesaba başlangıç bakiyesi — sonra /daily ile artırılabilir
      createdAt: serverTimestamp(),
      lastSeen: serverTimestamp()
    });
  } else {
    const data = existing.data();
    const patch = { lastSeen: serverTimestamp(), status: "online" };
    // Eski/eksik profillerde tag ya da username yoksa otomatik tamamla
    if (!data.tag) patch.tag = generateTag();
    if (!data.username) {
      patch.username = (displayNameOverride || user.displayName || user.email.split("@")[0])
        .toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 20) || "kullanici";
    }
    // Admin e-postası ile giriliyorsa ama eski kayıtta isAdmin eksik/false kalmışsa tamamla
    if (user.email === ADMIN_EMAIL && data.isAdmin !== true) patch.isAdmin = true;
...
export async function updateAvatarEmoji(emoji) {
  const user = auth.currentUser;
  if (!user) return;
  await updateDoc(doc(db, "users", user.uid), { avatarEmoji: emoji });
}
