...
const DEV_MODE_SHOW_CODE = true; // GEÇİCİ — email sorunu çözülene kadar açık kalsın
...
async function sendVerificationCode() {
  ...
  let mailDocRef;
  try {
    mailDocRef = await db.collection('mail').add({
      to: [pendingUser.email],
      requestedBy: pendingUser.uid,
      message: {
    ...
    console.error('Mail koleksiyonuna yazılamadı:', err);
    showMessage('Kod gönderilemedi (yazma hatası): ' + err.message);
  }

  document.getElementById('code-sent-to').textContent = `${pendingUser.email} adresine kod gönderiliyor...`;
  showStep(stepCode);

  // Trigger Email eklentisi işlemi bitirince bu belgeye "delivery" alanı
  // ekler — başarılı mı, hata mı olduğunu buradan gerçekten görebiliriz.
  if (mailDocRef) {
    const unsub = mailDocRef.onSnapshot(doc => {
      const data = doc.data();
      if (!data || !data.delivery) return;

      if (data.delivery.state === 'SUCCESS') {
        document.getElementById('code-sent-to').textContent = `${pendingUser.email} adresine kod gönderildi.`;
        unsub();
      } else if (data.delivery.state === 'ERROR') {
        document.getElementById('code-sent-to').textContent = 'Kod gönderilemedi.';
        showMessage('E-posta gönderim hatası: ' + (data.delivery.error || 'bilinmeyen hata') +
          ' — Firebase Console > Extensions > Trigger Email ayarlarını kontrol et.');
        unsub();
      }
    });
  }
