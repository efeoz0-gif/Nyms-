...
function renderTextChannel(contentEl, channelId, name) {
  contentEl.innerHTML = `
    ...
    <div id="channel-reply-banner" class="reply-banner hidden">
      <span></span>
      <button class="btn btn-ghost btn-sm" id="channel-reply-cancel">✕</button>
    </div>
    <div style="display:flex;gap:8px;">
      <input type="text" id="channel-message-input" class="code-input" style="font-size:14px;letter-spacing:0;text-align:left;margin:0;" placeholder="Mesaj yaz... (**kalın**, ||spoiler||)">
  ...
  let channelReplyingTo = null;
  const channelMsgCache = {};
  ...
  msgsRef.orderBy('createdAt').limitToLast(50).onSnapshot(snap => {
    ...
    snap.forEach(doc => {
      ...
      channelMsgCache[doc.id] = { text: m.text || '' };
      ...
      if (m.deleted) {
        ...
        let replyHtml = '';
        if (m.replyTo && channelMsgCache[m.replyTo]) {
          replyHtml = `<div class="reply-preview">↩️ ${escapeHtml(channelMsgCache[m.replyTo].text.slice(0, 60))}</div>`;
        }
        wrap.innerHTML = `
          ${replyHtml}
          <p style="font-size:13.5px;margin-bottom:2px;">
            <strong>${m.authorName || '?'}:</strong> <span class="msg-bubble-text">${renderRichText(m.text || '')}</span>
      ...
      if (!m.deleted) {
        attachMessageActions(wrap, {
          ref: doc.ref,
          isOwn: isMe,
          encryptFn: null, // sunucu kanalları şifreli değil
          getRawTextForEdit: async () => m.text || '',
          onReply: (msgId) => {
            channelReplyingTo = msgId;
            const banner = document.getElementById('channel-reply-banner');
            banner.classList.remove('hidden');
            banner.querySelector('span').textContent = '↩️ Yanıtlanıyor: ' + (m.text || '').slice(0, 50);
          }
        });
      }
    });
  ...
  document.getElementById('channel-reply-cancel').addEventListener('click', () => {
    channelReplyingTo = null;
    document.getElementById('channel-reply-banner').classList.add('hidden');
  });
  ...
  document.getElementById('btn-send-channel-msg').addEventListener('click', async () => {
    ...
    if (channelReplyingTo) msgData.replyTo = channelReplyingTo;

    await msgsRef.add(msgData);
    channelReplyingTo = null;
    document.getElementById('channel-reply-banner').classList.add('hidden');
  });
}
