...
import {
  ...
  query, where, onSnapshot, serverTimestamp, arrayUnion, arrayRemove, Timestamp
...
export async function joinServerByInvite(inviteCode) {
  ...
  const banSnap = await getDoc(doc(db, "servers", serverDoc.id, "bannedUsers", user.uid));
  if (banSnap.exists()) throw new Error("Bu sunucudan banlandınız");
...
// Banlama = at + kalıcı olarak "bannedUsers" listesine ekle (tekrar davetle giremesin diye).
// Sadece sunucu sahibi çağırabilir (UI'da kısıtlanır, ayrıca Rules da bannedUsers yazımını owner'a kısıtlar).
export async function banFromServer(serverId, uid) {
  await deleteDoc(doc(db, "servers", serverId, "members", uid)).catch(() => {});
  await setDoc(doc(db, "servers", serverId, "bannedUsers", uid), { bannedAt: serverTimestamp() });
}
export async function unbanFromServer(serverId, uid) {
  await deleteDoc(doc(db, "servers", serverId, "bannedUsers", uid));
}
export async function isBannedFromServer(serverId, uid) {
  const snap = await getDoc(doc(db, "servers", serverId, "bannedUsers", uid));
  return snap.exists();
}

// Zamanlı susturma — belirtilen dakika kadar o kişi o sunucuda mesaj gönderemez
export async function muteMemberTimed(serverId, uid, minutes) {
  const until = Timestamp.fromMillis(Date.now() + minutes * 60 * 1000);
  await updateDoc(doc(db, "servers", serverId, "members", uid), { mutedUntil: until });
}
export async function unmuteMember(serverId, uid) {
  await updateDoc(doc(db, "servers", serverId, "members", uid), { mutedUntil: null });
}

// Sunucuya özel admin rolü ver/al (owner hariç — owner rolü asla değişmez)
export async function setMemberRole(serverId, uid, role) {
