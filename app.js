...
auth.onAuthStateChanged(async (user) => {
  ...
  // Sadece moderatörse "Moderasyon" menü öğesini göster
  const isMod = await checkIsModerator(currentUser.uid, currentUser.email);
  if (isMod) {
    document.getElementById('nav-moderation').classList.remove('hidden');
  }
});

document.getElementById('nav-moderation').addEventListener('click', () => openAdminPanel());

// ---- Nav geçişi — gerçek view değiştirme ----
const VIEW_IDS = ['feed', 'servers', 'settings', 'chat'];
document.querySelectorAll('.nav-item[data-view]').forEach(item => {
  item.addEventListener('click', () => {
    ...
    // Diğer view'lar (stories, soundboard vb.) henüz bu sürümde
...
// ---- Doğum günü özelliği kaldırıldı ----

// ---- Seri (Streak) Sistemi — gerçek zamanlı ----
