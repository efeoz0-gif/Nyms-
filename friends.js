...
// --- Sadece kullanıcı adıyla arama (composer'daki @bahsetme "#tag" içermediği için;
// para komutlarında @isim yazınca da bulunabilsin diye) ---
export async function findUserByUsername(username) {
  const clean = username.replace("@", "").split("#")[0].toLowerCase();
...
export async function sendFriendRequest(targetUid) {
  ...
  await setDoc(doc(db, "friendRequests", reqId), {
    ...
    fromAvatarEmoji: meProfile.data().avatarEmoji || null,
    status: "pending",
